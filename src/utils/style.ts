...
type SpaceThemeKeys = keyof typeof themes.space
type ColorsThemeKeys = keyof typeof themes.colors
type FontSizesThemeKeys = keyof typeof themes.fontSizes
type LetterSpacingsThemeKeys = keyof typeof themes.letterSpacings
    ...
    if (isResponsivePropType(prop)) {
        ...
        for (const responsiveKey in prop) {
            if (responsiveKey === 'base') {
                ...
                result.push(`@media screnen and (min-width: ${breakpoint}) {${style}}`)
            }
        }
        return result.join('\n')
    }
    return `${propKey}: ${toThemeValueIfNeeded(propKey, prop, themes)};`
}
...
function isColorThemeKeys(prop: any, theme: AppTheme): prop is SpaceThemeKeys {
    return Object.keys(theme.colors).filter((key) => key == prop).length > 0
}

function isFontSizeThemeKeys(prop: any, theme: AppTheme): prop is SpaceThemeKeys {
    return Object.keys(theme.colors).filter((key) => key == prop).length > 0
}

function isLetterSpacingThemeKeys(prop: any, theme: AppTheme): prop is SpaceThemeKeys {
    return Object.keys(theme.letterSpacings).filter((key) => key == prop).length > 0
}

function isLineHeightThemeKeys(prop: any, theme: AppTheme): prop is SpaceThemeKeys {
